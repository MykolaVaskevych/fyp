"""Generate publication-quality figures for the Atari experiment (Experiment 3).

Produces 15 PNGs:
  1.  learning_curves_pong.png        — mean±std + median±IQR (1×2 subplots)
  2.  learning_curves_breakout.png    — same for Breakout
  3.  learning_curves_seaquest.png    — same for Seaquest
  4.  combined_learning_curves.png    — 3-row stack (DQN vs PPO, all envs)
  5.  score_distribution.png          — 3-row boxplot+swarm
  6.  per_seed_heatmap.png            — 1×2 algo panels
  7.  per_seed_boxswarm.png           — 3-row box+swarm with anomaly marks
  8.  final_performance.png           — grouped bars (IQM/Mean/Median + 95% CI)
  9.  performance_profile.png         — DQN vs PPO curves + CI bands
  10. optimality_gap.png              — horizontal bars + CI
  11. poi_heatmap.png                 — 2×2 annotated heatmap
  12. timing_analysis.png             — per-env training time bars
  13. sample_efficiency_pong.png      — IQM over training + CI bands
  14. sample_efficiency_breakout.png  — same for Breakout
  15. sample_efficiency_seaquest.png  — same for Seaquest
...
ENV_SLUGS = ["pong", "breakout", "seaquest"]
ENV_LABELS = {
    "pong": "Pong",
    "breakout": "Breakout",
    "seaquest": "Seaquest",
...
def plot_combined_learning_curves() -> None:
    ...
    n_envs = len(ENV_SLUGS)
    fig, axes = plt.subplots(n_envs, 1, figsize=(10, 4 * n_envs), sharex=False)
...
def plot_score_distribution() -> None:
    ...
    n_envs = len(ENV_SLUGS)
    fig, axes = plt.subplots(n_envs, 1, figsize=(8, 3.5 * n_envs), sharex=True)
    for i, slug in enumerate(ENV_SLUGS):
        ax = axes[i]
        data = []
        labels = []
...
def plot_timing_analysis() -> None:
    ...
    for ai, algo in enumerate(ALGO_ORDER):
        ...
        times = []
        for slug in ENV_SLUGS:
            t = timing.get(slug, {}).get("total_wall_seconds", 0)
            times.append(t)
            if t > 0:
                has_data = True
...
if __name__ == "__main__":
    ...
    plot_learning_curves_single("seaquest")
    ...
    plot_sample_efficiency_single("seaquest")
    print("\nDone — 15 figures regenerated.")
