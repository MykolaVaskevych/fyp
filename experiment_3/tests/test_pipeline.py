"""Smoke tests for the Atari evaluation pipeline (Experiment 3)."""
...
class TestEnvConfig:
    ...
    def test_master_seed_differs_from_previous(self):
        assert MASTER_SEED == 20260301
        assert MASTER_SEED != 20260212  # Experiment 1's seed
        assert MASTER_SEED != 20260215  # Experiment 2's seed
    ...
    def test_registry_structure(self):
        ...
        for env_name in ENV_ORDER:
            ...
            assert spec.total_timesteps == 20_000_000
            assert spec.slug
            assert spec.action_space_type == "discrete"
            assert spec.n_envs == 16

    def test_three_envs(self):
        assert len(ENV_ORDER) == 3
        assert set(ENV_ORDER) == {"Pong", "Breakout", "Seaquest"}

    def test_seaquest_spec(self):
        spec = ENV_REGISTRY["Seaquest"]
        assert spec.env_id == "SeaquestNoFrameskip-v4"
        assert spec.slug == "seaquest"
        assert spec.max_return == 42054.7
        assert spec.random_baseline == 68.4
        assert spec.total_timesteps == 20_000_000
        assert spec.eval_freq == 100_000
        assert spec.n_envs == 16
        assert spec.n_eval_episodes == 10
        assert spec.action_space_type == "discrete"
